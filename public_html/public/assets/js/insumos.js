...
function selecionObraTerminado (data) {
	...
	var numm = data.proveedores.length;
	var p = "<option value=''>Seleccione un proveedor</option>";
	...
	for (var j = 0; j < numm; j++) {
		p += "<option value='"+data.proveedores[j].id+"'>"+data.proveedores[j].nombre+"</option>";	
	}
	$("#estimacion").html(e);
	$("#proveedor").html(p)
	//console.log(e);
